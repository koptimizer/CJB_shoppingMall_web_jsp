...
<title>JOIN US</title>
<link href="layout_join.css?ver=14" rel="stylesheet" type="text/css" />
...
/*
 * 
 http://ontheinfo.com/2948*/
 
// 휴대전화 입력 시 커서 이동 함수
function moveCursor1(textbox){
	 if(textbox.value.length == 3){
		 document.getElementById("memberTel2").focus();
	}
}

function moveCursor2(textbox){
	 if(textbox.value.length == 4){
		 document.getElementById("memberTel3").focus();
		 }
}

// 가입 전 유효성 검사
function checkJoin(){
	var chk1 = document.form1.agreement1.checked;
	var chk2 = document.form1.agreement2.checked;
	...
	// 아이디 길이
	if(f.memberId.value.length < 4 || f.memberId.value.length > 12){
		alert("아이디는 4~12 자 이내로 가능합니다"); 
		f.memberId.focus();
		return false;
	}
	// 아이디 숫자로 시작할 경우
	
	if(!isNaN(f.memberId.value.substr(0,1))){
		alert("아이디는 숫자로 시작 할 수 없습니다"); 
		f.memberId.focus();
		f.memberId.select();
		return false;
	}
	...
	// 아이디 형식 검사
	 for (var i = 0; i < document.form1.memberId.value.length; i++) {
	     ch = f.memberId.value.charAt(i)
	     if (!(ch >= '0' && ch <= '9') && !(ch >= 'a' && ch <= 'z')) {
	         alert("아이디는 영문 소문자, 숫자만 입력가능 합니다.");
	         f.memberId.focus();
	         f.memberId.select();
	         return false;
	     }
	...
	// 비밀번호
	 if(f.memberPw.value == ""){
		...
		// 비밀번호 일치여부
	 if(f.memberPw.value != f.memberPwR.value){
		...
		// 비밀번호 형식 유효성 검사
	 var reg_pwd = /^.*(?=.{6,20})([a-zA-Z0-9])/;
	 if(!f.memberPw.value.match(reg_pwd)){
		 alert("비밀번호는 영문,숫자 혼합하여 6~20자 이내여야 합니다.");
		f.memberPw.focus();
		 f.memberPw.select();
	 ...
	 // 이메일
	 if(f.memberEmail1.value == "" || f.memberEmail2.value ==""){
		 alert("이메일을 입력해주세요.");
			f.memberEmail1.focus();
			f.memberEmail1.select();
			return false;
	 }
 ...
 // 이용약관
	 if(!chk1){
		alert("이용약관에 동의해주세요");
		document.agreement1.focus();
		document.agreement1.select();
		return false;		
	}
	 
	if(!chk2){
		alert("이용약관에 동의해주세요");
		document.agreement2.focus();
		document.agreement2.select();
...
<div id="wrap">
	...
	<div class="main_page">
			<div class="main_pro">
				<div class="main_box">
		<div class="form_box">
			<form id="form1" name="form1" method="post" action="process_joinResult.jsp">
				<table width="100%" border="0" cellspacing="0" cellpadding="3">
					<tr>
						<td width="17%">아이디</td>
						<td width="83%"><label for="memberId"></label>
					...
					<tr>
						<td>비밀번호확인</td>
						<td>
							<input type="password" name="memberPwR" id="memberPwR" /></td>
					</tr>
					<tr>
						<td>이름</td>
						<td>
							<input type="text" name="memberName" id="memberName" /></td>
					</tr>
					<tr>
						<td>우편번호</td>
						<td>
					...
					<tr>
						...
						<td valign="middle">
							<input type="text" name="memberTel1" id="memberTel1" onkeyup="moveCursor1(this)"/>
							&#8211;
							<input type="text" name="memberTel2" id="memberTel2" onkeyup="moveCursor2(this)"/>
							&#8211;
							<input type="text" name="memberTel3" id="memberTel3" /></td>
					</tr>
					<tr>
						<td>이메일</td>
						<td><input type="text" name="memberEmail1" id="memberEmail1" />
							@
							<input type="text" name="memberEmail2" id="memberEmail2" value=""/></td>
					</tr>
				</table>				
				<div class="term_box">
					이용약관 동의
					<div class="agree_box">
							 <P>제1조(목적)<BR>
											 이 약관은 천자비 회사(전자상거래 사업자)가 운영하는 천자비사이버 몰(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리?의무 및 책임사항을 규정함을 목적으로 합니다.</P>
<P>※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다」</P>
<P>제2조(정의)<BR>①“몰” 이란 천자비회사가 재화 또는 용역(이하 “재화등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터등 정보통신설비를 이용하여 재화등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.</P>
...
<P>제14조(환급)<BR>“몰”은 이용자가 구매신청한 재화등이 품절 등의 사유로 인도 또는 제공을 할 수 없을 때에는 지체없이 그 사유를 이용자에게 통지하고 사전에 재화 등의 대금을 받은 경우에는 대금을 받은 날부터 2영업일 이내에 환급하거나 환급에 필요한 조치를 취합니다.</P>
...
부 칙(시행일) 이 약관은 2019년 11월 30일부터 시행합니다.    
											</p>
					</div>
					<p>&nbsp;&nbsp;&nbsp;약관에 동의하시겠습니까?&nbsp;
						<input type="checkbox" name="agreement1" id="agreement"/> 동의함 </p>
				</div>
				<div class="term_box">
					개인정보 수집 및 이용 동의
					<div class="agree_box">
					...
					<p>&nbsp;&nbsp;&nbsp;이용약관에 동의하시겠습니까?&nbsp;
               			<input type="checkbox" name="agreement2" id="agreement"/> 동의함 </p>
				</div>
				
				<input type="button" name="back" id="back" value="&laquo; Back" onclick="location.href='login.jsp'"/>
				
				<div class="button_box">	
					<!-- 그 부분 -->					
					<input type="button" id="gaib" value="가입하기" onclick="checkJoin()"/>
					&nbsp;
					<input type="reset" name="reset" id="reset" value="지우기" /> 									
				</div>
...
<div class="main_footer">
				<table width="70%" border="0" cellspacing="0" cellpadding="3">
					<tr>
						<td width="20%"><a href="../mainPage/main.jsp">HOME</a></td>
						<td width="20%"><a href="../subPage/info.jsp">ABOUT US</a></td>
						<td width="20%"><a href="../subPage/agreement.jsp">AGREEMENT</a></td>
						<td width="20%"><a href="../subPage/privacy.jsp">PRIVACY POLICY</a></td>
						<td width="20%"><a href="../subPage/guide.jsp">SHOP GUIDE</a></td>
					</tr>
				</table>
				<hr/>
				<table width="70%" border="0" cellspacing="0" cellpadding="3">
					<tr>
						<th width="20%">CS CENTER</th>
						<th width="20%">BANK INFO</th>
					</tr>
					<tr>
						<td>032-425-6624</td>
						<td>국민 307002-04-301224 고광종(천자비)</td>
					</tr>
					<tr>
						<td>MON-SAT 9:00-11:00 PM&nbsp;SUN 10:00-7:00 PM</td>
						<td>&nbsp;</td>
					</tr>
				</table>
				<hr/>
				COMPANY NAME 천자비 OWNER 고광종 PRIVACY OFFICER 고광종 E-MAIL rhrhkdwhd@naver.com<br />
			ADDRESS 인천광역시 부평구 십정2동 390-26</div>
		</div>
	</div>
</div>
</div>
